...
面向瑜伽空间的互动机器狗项目。V1 包含瑜伽垫感知、用户定位与跟随、下犬式识别和拍照、计时，以及下犬式组合、俯卧撑、坐下注视和固定口令启动反馈。代码采用可替换接口，允许先用录像和 Mock 完成验证，再接入厂商正式 SDK。

目标真机已确定为 **Vbot 超能机器狗“大头 EDU 版”**。项目版本 `0.4.1` 使用“瑜伽”关键词触发 V1 启动反馈；具体固件、SDK、ROS 2 发行版与接口名称必须从交付设备读取后填写，不能用公开参数代替实测。
...
- `docs/devices/VBOT_EDU_V1_ACTION_MAPPING.md`：V1 动作、屏幕和语音到真机接口的放行状态。
